...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "kck" is a composite category enabling all KackCoin-related debug output
            if (ptrCategory->count(string("kck"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "kck.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty kck.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override kck.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kckd.pid"));
